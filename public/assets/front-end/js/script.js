...
if (fullscreenBtn && videoContainer && video) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function handleNavbarScroll() {
    ...
    if (scrollPosition > scrollThreshold) {
      ...
      if (logoText) logoText.classList.replace("text-white", "text-slate-800");

      navLinks.forEach((link) => {
        link.classList.replace("text-white/90", "text-slate-600");
        link.classList.replace("hover:text-white", "hover:text-primary");
      });
      ...
      if (logoText) logoText.classList.replace("text-slate-800", "text-white");

      navLinks.forEach((link) => {
        link.classList.replace("text-slate-600", "text-white/90");
        link.classList.replace("hover:text-primary", "hover:text-white");
      });
