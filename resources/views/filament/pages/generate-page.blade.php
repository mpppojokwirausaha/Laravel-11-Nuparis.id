...
<div x-data="{
    ...
    fieldStyle(key, field) {
        ...
        // preview, supaya perubahan angka di input "Font" benar-benar terlihat di kanvas. const canvasWidthPx=this.$refs.canvasWrap ?
    this.$refs.canvasWrap.getBoundingClientRect().width : 0; const ptToPx=(this.pdfWidthPt && canvasWidthPx) ?
    (canvasWidthPx / this.pdfWidthPt) : 1; const previewFontSize=Math.max(8, Math.round(field.font_size * ptToPx));
    const widthCh=Math.max(field.label.length, 6) + 2; const fontWeight=field.font_bold ? '700' : '600' ; const
    textDecoration=field.font_underline ? 'underline' : 'none' ; const cssFontMap={
    Helvetica: "'Helvetica', 'Arial', sans-serif" , Times: "'Times New Roman', Times, serif" ,
    Courier: "'Courier New', Courier, monospace" , }; const cssFontFamily=cssFontMap[field.font_family] ||
    cssFontMap.Helvetica; return `position:absolute; left:${field.x}%; top:${field.y}%; min-width:${widthCh}ch;
    transform: translate(-50%, -50%); cursor:grab; touch-action:none; border:2px dashed #dc2626;
    background-color:rgba(220,38,38,0.15); box-sizing:border-box; padding:6px 10px; border-radius:6px; display:flex;
    align-items:center; justify-content:center; text-align:center; line-height:1.1; font-family:${cssFontFamily};
    font-size:${previewFontSize}px; font-weight:${fontWeight}; text-decoration:${textDecoration};
    color:${field.font_color || '#dc2626' }; white-space:nowrap; user-select:none; z-index:50;`; }, hasUnplacedFields()
    ...
    @pointercancel.window="stopDrag($event.clientX, $event.clientY)">
            ...
            @if ($previewImageUrl)
                <div x-ref="canvasWrap"
                    ...
                    <template x-for="(field, key) in fields" :key="key">
                        <div x-show="field.is_placed" @pointerdown="startDrag(key, $event)"
                            :style="fieldStyle(key, field)" x-text="key === 'qrcode' ? '' : field.label"></div>
                    </template>
                </div>
            @else
